	...
	if SERVER then
		if ( self.Owner:KeyDown( IN_ATTACK ) && self.Owner:isWepRaised() && !self:GetDTBool(3) ) then
			if self.timeCooldown < CurTime() then
				...
				local redc = ( self.Owner:getChar():getAttrib("end", 0) / 100 )
...
function SWEP:FireBullet( spr, ang, hit )
		...
		if SERVER then
			...
			dam = dam * ( 1 + self.Owner:getChar():getAttrib("str", 0)/100 )
			dam = dam / 3
		end
...
function SWEP:Impact( trace )
	...
	if (nut.config.get("gathering", false) and CLIENT) then
		netstream.Start("nut_lc_gather", entity, self)
	else
...
function SWEP:DoDamage()
	...
	if SERVER then
		local stam = self.Owner:getLocalVar( "stm", 0 )
		local redc = ( self.Owner:getChar():getAttrib("end", 0) / 100 )
		local value = math.Clamp(stam - ( self.Primary.Stamina - redc * self.Primary.Stamina ), 0, 100)
		self.Owner:setLocalVar( "stm", value )
	end
	local ang = self.Primary.Angle
	local spr = self.Primary.Spread
	local tracedata = {}
